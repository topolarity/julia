...
let (bt, did_gc) = make_oc_and_collect_bt()
    ...
    @test did_gc[]
    # The body frame must resolve to the OC's unregistered method through the
    # executing CodeInstance (kept alive for backtraces by its global-root
    # promotion at JIT registration), not merely to the method the transplanted
    # IR was inferred from.
