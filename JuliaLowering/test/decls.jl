...
@testset "(AI) global/local method def on a qualified name" begin
    # From PkgEval: MLJGLMInterface v0.3.7, via StatisticalMeasures src/continuous.jl:13 (@combination emits global Mod.f(...) = ... inside let)
...
@testset "(AI) all-underscore rvalue in a local/global decl reports the specific message" begin
    # From PkgEval: Chain v1.0.0, test/runtests.jl:222 (asserts the write-only-underscore message for @chain begin _ end)
