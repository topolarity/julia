...
vst1(vcx::Validation1Context, st::SyntaxTree)::ValidationResult = @stm st begin
    ...
    ([K"boundscheck" [K"Value"]], when=(getattr(Any, st[1], :value) isa Bool)) -> pass()
    ([K"inbounds" [K"Value"]], when=(getattr(Any, st[1], :value) isa Bool)) -> pass()
    ([K"inbounds" [K"Identifier"]], when=(getattr(String, st[1], :name_val) == "pop")) -> pass()
    ([K"inline" [K"Value"]], when=(getattr(Any, st[1], :value) isa Bool)) -> pass()
    ([K"noinline" [K"Value"]], when=(getattr(Any, st[1], :value) isa Bool)) -> pass()
    [K"purity"] -> pass()
...
function vst1_importpath(vcx, st; dots_ok)
    ...
    for c in path_components
        if kind(c) === K"Identifier" && getattr(String, c, :name_val) === "."
...
vst1_calldecl_dot_name_rhs(vcx, st) = @stm st begin
    ...
    ([K"Value"], when=getattr(Any, st, :value) isa String) -> _ident_str(vcx, st, getattr(Any, st, :value); lhs=true)
    [K"String"] -> _ident_str(vcx, st, getattr(Any, st, :value); lhs=true)
...
vst1_calldecl_name(vcx, st) = @stm (st=strip_arg_meta(st)) begin
    [K"Identifier"] -> vst1_ident(vcx, st; lhs=true) &
        (!is_dotted_operator(getattr(String, st, :name_val)) ? pass() :
...
strip_arg_meta(st) = @stm st begin
    [K"meta" s arg] -> let meta_s = getattr(String, s, :name_val, "")
        kind(arg) === K"meta" ? st :
...
vst0_macrocall(vcx, st) = @stm st begin
    ...
    ([K"macrocall" name [K"Value"] args...],
     when=(typeof(getattr(Any, st[2], :value)) in (LineNumberNode, MacroSource))) ->
         pass()
    [K"macrocall" _...] ->
...
function _assert_syntaxtree(st::SyntaxTree, parents::Vector{NodeId}, vr)
    ...
    getattr(SourceAttrType, st, :source) === st._id && (vr &= @fail(st, ".source equal to self ID"))
    getattr(NodeId, st, :macro_source, nothing) === st._id &&
