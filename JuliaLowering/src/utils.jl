...
function _value_string(ex)
    ...
    str = k == K"Identifier"  ? getattr(String, ex, :name_val)           :
          ...
          k == K"oldsymbolicgoto" ? "goto:$(getattr(String, ex, :name_val))" :
          k == K"SourceLocation" ?
...
function _show_provtree(io::IO, ex::SyntaxTree, indent)
    ...
    src = getattr(SourceAttrType, ex, :source)
    msrc = getattr(NodeId, ex, :macro_source, nothing)
    ...
    if src isa NodeId
        ...
        @jl_assert getattr(SourceAttrType, ex, :source) isa Union{LineNumberNode, SourceRef} ex
        src = sourceref(ex)
        fn = filename(src)
