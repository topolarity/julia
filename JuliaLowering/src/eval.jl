...
@fzone "JL: eval" function eval(mod::Module, @nospecialize(ex);
                                ...
                                soft_scope::Union{Nothing,Bool}=nothing,
                                opts...)
    # `eval` is emitted into user code by `@eval` and runs at the caller's latest
    # world, where methods defined by the surrounding code would invalidate (and
    # re-infer) driver code.
    #
    # Instead, dispatch the whole driver at the pinned lowering world. Operations
    # with user-visible world semantics are required to dispatch back to the latest
    # world inside `_eval`.
    return invoke_in_lowerer_world(_lower_and_eval, mod, ex, soft_scope,
                                   NamedTuple(opts))
end

# `ex` may be a `SyntaxTree` or an `Expr` (or `Expr` tree leaves of any type).
function _lower_and_eval(mod::Module, @nospecialize(ex),
                         soft_scope::Union{Nothing,Bool}, opts::NamedTuple)
    st = ex isa SyntaxTree ? ex : expr_to_est(ex)
    iter = lower_init(st; opts...)
