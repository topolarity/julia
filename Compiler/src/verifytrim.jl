...
function get_verify_typeinf_trim(codeinfos::Vector{Any})
    ...
    for i = 1:length(codeinfos)
        ...
        elseif item isa SimpleVector
            ...
            # coverage, which is unavailable under --trim. We could implement this but
            # we choose not to because of the implicitly degraded ABI (better to point
            # this out to the user instead of leaving `@ccallable` allocating etc)
