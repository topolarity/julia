...
"""
    generated_function_stub_layout(generator) -> (gen, firstarg) or nothing

Return the code-generator callable of a generated-function stub together with
the number of leading slots in its (single) method that precede the
static-parameter slots (`1` for `Core.GeneratedFunctionStub`: just `#self#`).
Alternative lowering implementations extend this for their own stub types so
that reflection (see [`may_invoke_generator`](@ref)) can reason about which
arguments the generator uses. Returns `nothing` for unknown stub types.
"""
function generated_function_stub_layout(@nospecialize(generator))
    isa(generator, Core.GeneratedFunctionStub) && return generator.gen, 1
    return nothing
end
...
function may_invoke_generator(method::Method, @nospecialize(atype), sparams::SimpleVector)
    ...
    if isa(generator, Core.GeneratedFunctionStub)
        # Fast path for the frontend's own stubs: no world switch.
        gen, firstarg = generator.gen, 1
    else
        # An alternative lowering's layout method postdates this code, so it
        # can be invisible in the ambient world: inference runs at the pinned
        # typeinf world, which predates anything defined after the base
        # sysimage was built (JuliaLowering added by an incremental sysimage
        # build, or loaded as a package). The stub-carrying method was
        # necessarily defined by that lowering, so its `primary_world` always
        # sees the layout method — dispatch there, mirroring
        # `jl_code_for_staged`, which invokes the generator itself in
        # `def->primary_world`.
        layout = invoke_in_world(method.primary_world, generated_function_stub_layout, generator)
        layout === nothing && return false
        gen, firstarg = layout
    end
    tt = Tuple{typeof(gen), Vararg{Any}}
    ...
    (nslots >= firstarg + length(sparams) + length(at.parameters)) || return false
