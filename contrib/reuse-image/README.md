...
Reuse can only be as good as image-code validity. Attribution with
whoinvalidates.jl found and fixed three sources that re-derived thousands of
CIs on every `using Plots`:
...
- whoinvalidates.jl — invalidation attribution: triggers ranked by total
  victims, per-module breakdown, and direct (depth-0) victim listing for a
  focused trigger.
