...
# compiler-rt is required for linking sysimages on Darwin; place it at the same path the JLL uses
LLVM_INSTALL += && mkdir -p $2$$(build_libdir)/darwin && install -m 0644 $2$$(build_prefix)/lib/clang/$$(LLVM_VER_MAJ)/lib/darwin/libclang_rt.osx.a $2$$(build_libdir)/darwin/libclang_rt.osx.a
endif
ifeq ($(BUILD_LLD), 1)
...
ifeq ($(OS),Darwin)
# compiler-rt is required for linking sysimages on Darwin; the JLL extracts it to $(build_libdir)/darwin/
