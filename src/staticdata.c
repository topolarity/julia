...
static void jl_queue_module_for_serialization(jl_serializer_state *s, jl_module_t *m) JL_GC_DISABLED
{
    ...
    if (s->heap_snapshot) {
        gc_heap_snapshot_record_object_edge((jl_value_t*)m, (jl_value_t**)&m->name);
        gc_heap_snapshot_record_object_edge((jl_value_t*)m, (jl_value_t**)&m->parent);
        gc_heap_snapshot_record_object_edge((jl_value_t*)m, (jl_value_t**)&m->bindings);
        gc_heap_snapshot_record_object_edge((jl_value_t*)m, (jl_value_t**)&m->bindingkeyset);
        if (!jl_options.strip_metadata)
            gc_heap_snapshot_record_object_edge((jl_value_t*)m, (jl_value_t**)&m->file);
    }
    ...
    for (size_t i = 0; i < module_usings_length(m); i++) {
        ...
        // FIXME: it is questionable to use array indices and bypass the intermediate
        //        array here in the object graph, although it is what the main heap
        //        snapshot utility does in the GC
    ...
    if (jl_options.trim || jl_options.strip_ir) {
        ...
        if (s->heap_snapshot) {
            gc_heap_snapshot_record_binding_partition_edge(
                (jl_value_t*)m, jl_atomic_load_relaxed(&m->usings_backedges));
            gc_heap_snapshot_record_binding_partition_edge(
                (jl_value_t*)m, jl_atomic_load_relaxed(&m->scanned_methods));
        }
    }
}
    ...
    if (layout->npointers == 0) {
        ...
        if (jl_genericmemory_how(m) == 3) {
            ...
            for (i = 0; i < l; i++) {
                ...
                if (s->heap_snapshot && fld) {
                    _gc_heap_snapshot_record_array_edge(v, fld, gc_slot_to_arrayidx(v, fld_ptr));
                }
            }
        }
...
static void record_gvars(jl_serializer_state *s, arraylist_t *globals) JL_GC_DISABLED
{
    for (size_t i = 0; i < globals->len; i++) {
        ...
        if (s->heap_snapshot) {
            gc_heap_snapshot_record_gc_roots((jl_value_t*)globals->items[i], "gvar");
        }
    }
}
...
#undef XX
        ...
        if (s.incremental) {
            ...
            if (s.heap_snapshot) {
                gc_heap_snapshot_record_root((jl_value_t*)s.method_roots_list, "method_roots_list");
            }
            jl_serialize_reachable(&s);
        }
        ...
        if (worklist == NULL) {
            ...
            if (s.heap_snapshot) {
                gc_heap_snapshot_record_root((jl_value_t*)global_roots_list, "global_roots_list");
                gc_heap_snapshot_record_root((jl_value_t*)global_roots_keyset, "global_roots_keyset");
            }
            jl_serialize_reachable(&s);
        }
