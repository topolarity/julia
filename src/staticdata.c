...
static void jl_queue_module_for_serialization(jl_serializer_state *s, jl_module_t *m) JL_GC_DISABLED
{
    ...
    jl_queue_for_serialization(s, (jl_value_t*)m->package);
}
...
static void jl_write_module(jl_serializer_state *s, uintptr_t item, jl_module_t *m) JL_GC_DISABLED
{
    ...
    newm->package = NULL;
    arraylist_push(&s->relocs_list, (void*)(reloc_offset + offsetof(jl_module_t, package)));
    arraylist_push(&s->relocs_list, (void*)backref_id(s, (jl_value_t*)m->package, s->link_ids_relocs));
