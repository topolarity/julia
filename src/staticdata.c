...
static void jl_insert_into_serialization_queue(jl_serializer_state *s, jl_value_t *v, int recursive, int immediate) JL_CANSAFEPOINT JL_GC_DISABLED
{
    ...
    if (jl_typetagis(v, jl_abi_adapter_cache_type)) {
        // Rebuilt from reinserted records, drop it for now.
...
#undef XX
    ...
    for (size_t i = 0; i < reinsert_objs.len; i++) {
        // Only completed records enter the cache; skip any whose thunk was not
        // wired (the JIT rebuilds them on demand).
