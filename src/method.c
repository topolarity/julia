...
// to an `AbstractSystemLibrary`, expand it to a 4-tuple
// `(fname, lib_ref, lib_id, lib_name)` so the ccall/cglobal codegen path
// can route the spec through the link-time native-link mechanism. Quiet
// no-op if the lib doesn't resolve to an AbstractSystemLibrary, if Libdl
// isn't loaded yet (the type/dispatch globals are NULL during bootstrap),
// or if dlid/dlname return `nothing`.
static void abstract_system_library_expand(jl_expr_t *e, jl_module_t *module,
                                           ...
                                           const char *kind)
{
    ...
    if (nargs_tuple != 2 || jl_abstractsystemlibrary_type == NULL ||
    ...
    if (lib_val != NULL && jl_isa(lib_val, jl_abstractsystemlibrary_type)) {
        JL_TRY {
        ...
        if (lib_id != NULL && lib_id != jl_nothing &&
            ...
            jl_expr_t *new_tuple = jl_exprn(tuple_sym, 4);
                                              ...
                                              int binding_effects)
{
    ...
    if (e->head == jl_foreigncall_sym) {
        ...
        abstract_system_library_expand(e, module, sparam_vals, "ccall");
    ...
    if (e->head == jl_foreignglobal_sym) {
        ...
        abstract_system_library_expand(e, module, sparam_vals, "cglobal");
    }
