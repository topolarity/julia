...
//
// In addition to the ABI and invokee, a bridge is characterized by its "internal"
// transformation, usually responsible for world-handling and arg0 unpacking
// (type-erasure).
//
// XXX: These responsibilities could be moved to the caller rather than the adapter
// side, but the special invocation behavior must exist somewhere which makes these
// ABIs distinct from the "standard" case.
typedef enum {
    JL_ABI_STD = 0,            // `sigt` describes the call frame as-is
    JL_ABI_OPAQUE_CLOSURE = 1, // OpaqueClosure body frame: slot 0 of `sigt` is the
                               // captures environment rather than the callee
} jl_abi_kind_t;
...
// Return a specptr appropriate for invoking `codeinst`; NULL means an ABIAdapter is required
// and does not yet exist. `*invokee`, when non-NULL, receives the CI / ABIAdapter the specptr
// was derived from.
