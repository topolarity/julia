...
extern JL_DLLEXPORT jl_value_t *jl_abstractsystemlibrary_type JL_GLOBALLY_ROOTED;
extern _Atomic(jl_typemap_entry_t*) call_cache[N_CALL_CACHE] JL_GLOBALLY_ROOTED;

// Native-link policy: friendly_names (per AbstractSystemLibrary.dlname) that
// the AOT pipeline should bind via direct external symbol references at link
// time, instead of via the lazy-stub mechanism. Populated before AOT codegen
// runs (e.g. by juliac's `--link-native=libzstd,...`). Process-local; not
// part of the sysimage. The IR generated by codegen is identical to the JIT
// case — the rewrite happens in aot_emit_ccall_stubs.
...
// Foreign-deps export: when set to a non-empty path, the AOT codegen pipeline
// emits a JSON manifest of every ccall/cglobal usage site to that path
// (juliac's `--export-foreign-deps=path`). Includes both native-linked and
// non-native-linked entries.
