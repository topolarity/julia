      ...
      (cond
       ...
       ((and (pair? sig) (eq? (car sig) 'call))
        (let* ((raw-typevars (or where '()))
               ...
               (farg    `(|::| |#self#| (call (core Typeof) ,name)))
               (argl    (fix-arglist (cons farg argl))))
          ;; qualified names (`interface M.foo(...)`) are dotted sym-refs, same
          ;; as for method definitions; the forward `(method ,name)` declaration
          ;; is skipped for them in interface-def-expr-
          (if (not (sym-ref? name))
