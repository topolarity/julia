    ...
    jl_value_t *source_, jl_value_t *captures, int do_compile, size_t world) JL_CANSAFEPOINT
{
    ...
    // The source may be a `Method` or, in the optimized form, a dispatch-resolved body
    // `CodeInstance` baked in by inlining (see handle_new_opaque_closure_call!), in which
    // case we reuse its already-compiled code and skip method resolution.
    ...
    // Reuse the supplied CodeInstance if it is already compiled; otherwise compile.
