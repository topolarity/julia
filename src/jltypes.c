...
void jl_init_types(void) JL_GC_DISABLED
{
    ...
    *(size_t*)jl_an_empty_string = 0;
    ...
    jl_typed_callable_type = (jl_unionall_t*)jl_new_datatype(jl_symbol("TypedCallable"), core, jl_function_type, tv,
        // The trailing unnamed field is the hidden dispatch trampoline record, which both
        // the specsig call site and the jlcall builtin reach the target through
        // (see jl_typed_callable_t in julia.h).
        jl_perm_symsvec(2, "f", ""),
        jl_svec(2, jl_any_type, jl_any_type),
