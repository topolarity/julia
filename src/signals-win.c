...
void __cdecl crt_sig_handler(int sig, int num)
{
    ...
    default: // SIGSEGV, SIGTERM, SIGILL, SIGABRT
        ...
        if (event_source != INVALID_HANDLE_VALUE) {
            ...
            FatalAppExitW(0, L"error: received fatal signal"
...
LONG WINAPI jl_exception_handler(struct _EXCEPTION_POINTERS *ExceptionInfo)
{
    ...
    if (ExceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_ILLEGAL_INSTRUCTION) {
        ...
        jl_fprint_sigill(&summary, ExceptionInfo->ContextRecord);
    }
    ...
    jl_safe_fprintf(&summary, " at 0x%Ix -- ", (size_t)ExceptionInfo->ExceptionRecord->ExceptionAddress);
    jl_print_native_codeloc(&summary, (uintptr_t)ExceptionInfo->ExceptionRecord->ExceptionAddress);

    ios_t detail;
    ios_mem(&detail, 1024);
    jl_fprint_critical_error(&detail, 0, 0, ExceptionInfo->ContextRecord, ct);

    // First print to STDERR
    ios_write_direct(ios_stderr, &summary);
    ios_write_direct(ios_stderr, &detail);
    ...
    if (event_source != INVALID_HANDLE_VALUE) {
        ...
        const wchar_t *strings[] = {
            ios_utf8_to_wchar(summary.buf),
            ios_utf8_to_wchar(detail.buf),
        };
        ReportEventW(
            event_source, EVENTLOG_ERROR_TYPE, /* category */ 0, /* event_id */ (DWORD)0xE0000000L,
           /* user_sid */ NULL, /* n_strings */ 2, /* data_size */ 0, strings, /* data */ NULL
        );

        ios_skip(&summary, -1); // remove null terminator
        ios_puts("See Application log in Event Viewer for more information.\n", &summary);
        ios_putc('\0', &summary);

        // TODO: Maybe don't skip jl_exit?
        FatalAppExitW(0, ios_utf8_to_wchar(summary.buf));
    }
