...
static void generate_cfunc_thunks(jl_codegen_output_t &out) JL_CANSAFEPOINT
{
    ...
    for (jl_dispatch_trampoline_t *tr : out.cfuncs) {
        ...
        jl_code_instance_t *codeinst = resolve_trampoline_invokee(abi, compiled_mi, latestworld);
                                      ...
                                      int external_linkage) JL_CANSAFEPOINT
{
    ...
    // Transfer emitted records to the caller while temporary_roots still protects them.
    if (emitted_adapters)
        for (auto &[rec, F] : out.adapter_funcs)
            jl_array_ptr_1d_push(emitted_adapters, (jl_value_t*)rec);
    ...
    for (auto &[rec, F] : out.adapter_funcs) {
        ...
        data->jl_fvar_map[(jl_value_t*)rec] = {0, (uint32_t)data->jl_sysimg_fvars.size()};
    }
