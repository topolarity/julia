...
typedef struct _jl_abi_adapter_t {
    ...
    size_t kind : 8;             // jl_abi_kind_t of the caller ABI

    // callee target
    jl_code_instance_t *ci;      // target CodeInstance; may be NULL for dispatcher adapters

    _Atomic(void*) fptr;         // JITed adapter address (not GC-tracked)
    // Bucket link; promotion may regroup records by `ci`.
...
typedef struct _jl_dispatch_trampoline_t {
    ...
    jl_value_t *last_invokee;    // Union{CodeInstance, ABIAdapter}; NULL if unresolved
    _Atomic(void*) fptr;         // copied from last_invokee, cached here for speed
...
typedef struct _jl_abi_adapter_cache_t {
    JL_DATA_TYPE
    jl_typemap_list_t cache;      // (sigt) -> jl_abi_adapter_t, hash-indexed by `ci`
// hidden fields:
...
typedef struct _jl_dispatch_trampoline_cache_t {
    JL_DATA_TYPE
    jl_typemap_list_t cache;      // (sigt) -> jl_dispatch_trampoline_t (never re-indexed)
// hidden fields:
