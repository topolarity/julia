...
// One record of the global ABI-adapter cache (`jl_abi_adapters->cache`): a JIT'd adapter
// thunk that bridges the caller ABI (from_abi) to a target CodeInstance. Cached so
...
typedef struct _jl_dispatch_trampoline_t {
    ...
    // Last resolved target, as a Union{CodeInstance, ABIAdapter} (NULL = unresolved): a bare
    // `CodeInstance` when the declared ABI matches its specptr (no adapter needed), otherwise
    // the `ABIAdapter` record whose `fptr` is the compiled thunk.
