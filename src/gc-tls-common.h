...
typedef struct {
    ...
    // Unrooted registry of objects requiring weak processing on death
    // (currently linked cancellation sources). Both collectors unlink dead
    // entries after marking and before reclaiming storage, retaining live
    // entries for subsequent collections.
