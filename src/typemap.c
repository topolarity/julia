...
// Return the first matching record in the list.
        ...
        void *key) JL_CANSAFEPOINT
{
    ...
    for (jl_value_t *e = head; e != NULL; e = jl_atomic_load_acquire(typemap_list_next(e, cfg->next_offset))) {
        JL_GC_PROMISE_ROOTED(e); // rooted by the cache (records are never removed)
        if (cfg->match(e, key))
            return e;
    }
    return NULL;
...
// Build a `len`-slot table holding `v` plus every record in `old` (a list or a
        ...
        jl_value_t *v, size_t len) JL_CANSAFEPOINT
{
    ...
    if (ok && jl_is_genericmemory(old)) {
        jl_genericmemory_t *old = (jl_genericmemory_t*)old;
        _Atomic(jl_value_t*) *slots = (_Atomic(jl_value_t*)*)old->ptr;
        for (size_t i = 0; ok && i < old->length; i++) {
        ...
        // representation keep a complete view of pre-existing records.
...
// Return the record matching (`sigt`, `match(key)`); NULL if absent. `hash` is the
...
// Insert an absent record. Caller holds the cache writelock.
