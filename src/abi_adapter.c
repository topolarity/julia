...
// A jl_typemap_list_t cache keyed by `sigt`; records within a sigt bucket are
...
// `rt` is compared by type-equality, which admits no sound hash; `ci` (compared by
// identity) dominates the bucket cardinality anyway.
...
static uintptr_t abi_adapter_hash(jl_value_t *rec) JL_NOTSAFEPOINT
{
    jl_abi_adapter_t *e = (jl_abi_adapter_t*)rec;
...
// (Re)set hidden cache state; used at creation and as a restore fixup.
...
// Return a directly compatible specptr, or describe the target to adapt.
// This only reads metadata and never invokes codegen.
        ...
        void **target, int *target_specsig, jl_callptr_t *invoke) JL_CANSAFEPOINT
{
    ...
    *invoke = NULL;
    ...
    // Do not wait for codegen; unpublished targets have no shortcut.
...
// Return the target's directly compatible specptr, if any.
...
// A miss is definitive only while holding the writelock.
        ...
        void **target, int *target_specsig, jl_callptr_t *invoke, jl_value_t **invokee) JL_CANSAFEPOINT
{
    ...
    if (e == NULL)
        return NULL;
    // Cached records always carry a valid fptr, set before publication.
...
// Allocate a detached cache record; the caller roots its key fields.
        ...
        void *fptr, jl_value_t **invokee) JL_CANSAFEPOINT
{
    ...
    e = abi_adapter_lookup(from_abi.sigt, from_abi.rt, codeinst,
            from_abi.specsig, from_abi.kind);
    if (e != NULL) {
        // Cached records always carry a valid fptr; keep the winner.
...
// Reinsert a materialized record, keeping any existing canonical record.
// Only completed records may enter the cache; the caller skips unwired ones.
