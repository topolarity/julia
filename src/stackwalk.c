...
static void jl_safe_print_codeloc(ios_t *s, const char* func_name, const char* file_name,
...
// inlined code.
void jl_print_native_codeloc(ios_t *s, uintptr_t ip) JL_NOTSAFEPOINT
{
    ...
    for (i = 0; i < n; i++) {
        jl_frame_t frame = frames[i];
        if (!frame.func_name) {
            jl_safe_fprintf(s, "unknown function (ip: %p)\n", (void*)ip);
        }
        else {
            jl_safe_print_codeloc(s, frame.func_name, frame.file_name, frame.line, frame.inlined);
            free(frame.func_name);
            free(frame.file_name);
        }
    }
...
static void jl_print_debugloc(ios_t *s, jl_debuginfo_t *debuginfo, jl_value_t *func, size_t ip, int inlined) JL_NOTSAFEPOINT
{
    ...
    if (edges_idx) {
        ...
        jl_print_debugloc(s, edge, NULL, stmt.pc, 1);
    }
    intptr_t ip2 = stmt.line;
    if (ip2 >= 0 && ip > 0 && (jl_value_t*)debuginfo->linetable != jl_nothing) {
        jl_print_debugloc(s, debuginfo->linetable, func, ip2, 0);
    }
    else {
        ...
        jl_safe_print_codeloc(s, func_name, file, ip2, inlined);
    }
}

// Print code location for backtrace buffer entry at *bt_entry
void jl_print_bt_entry_codeloc(ios_t *s, jl_bt_element_t *bt_entry) JL_NOTSAFEPOINT
{
    if (jl_bt_is_native(bt_entry)) {
        jl_print_native_codeloc(s, bt_entry[0].uintptr);
    }
        ...
        if (jl_is_code_info(code)) {
            ...
            jl_print_debugloc(s, src->debuginfo, def, ip + 1, 0);
        }
        else {
...
JL_DLLEXPORT void jl_gdblookup(void* ip)
{
    ...
    jl_print_native_codeloc(&s, (uintptr_t)ip);
    ios_close(&s);
}

// Print backtrace for current exception in catch block
JL_DLLEXPORT void jlbacktrace(ios_t *s) JL_NOTSAFEPOINT
{
    jl_task_t *ct = jl_current_task;
    if (ct->ptls == NULL)
        return;
...
JL_DLLEXPORT void jlbacktracet(ios_t *s, jl_task_t *t) JL_NOTSAFEPOINT
{
    jl_task_t *ct = jl_current_task;
    jl_ptls_t ptls = ct->ptls;
    jl_rec_backtrace(t);
...
JL_DLLEXPORT void jl_print_backtrace(ios_t *s) JL_NOTSAFEPOINT
{
    jlbacktrace(s);
}
