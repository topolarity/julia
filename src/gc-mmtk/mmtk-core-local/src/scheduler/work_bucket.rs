...
impl<VM: VMBinding> WorkBucket<VM> {
    ...
    /// Number of packets currently queued in this bucket (approximate; other
    /// threads may push/steal concurrently).  Used for proportional wakeups.
    ...
    pub fn bulk_add(&self, work_vec: Vec<Box<dyn GCWork<VM>>>) {
        ...
        // Proportional wakeup: for small batches, waking the whole herd costs
        // more in futex traffic and re-park mutex contention than the work
        // itself.  notify_one on a condvar with no waiters is nearly free.
        if !self.is_open() || !self.is_enabled() {
            return;
        }
        if n >= 8 {
