...
impl<C: GCWorkContext> GCWork<C::VM> for Prepare<C> {
    fn do_work(&mut self, worker: &mut GCWorker<C::VM>, mmtk: &'static MMTK<C::VM>) {
        ...
        if mmtk.get_plan().needs_collector_context_packets() {
            for (i, w) in mmtk.scheduler.worker_group.workers_shared.iter().enumerate() {
                let result = w.designated_work.push(Box::new(PrepareCollector));
                debug_assert!(result.is_ok());
                // Designated work is invisible to bucket polling: wake its
                // owner directly (targeted per-worker token wake).
                mmtk.scheduler.worker_monitor.notify_worker(i);
            }
        }
    }
}
    ...
    fn do_work(&mut self, worker: &mut GCWorker<C::VM>, mmtk: &'static MMTK<C::VM>) {
        ...
        if mmtk.get_plan().needs_collector_context_packets() {
            for (i, w) in mmtk.scheduler.worker_group.workers_shared.iter().enumerate() {
                ...
                debug_assert!(result.is_ok());
                // See Prepare::do_work: targeted wake for the owner.
                mmtk.scheduler.worker_monitor.notify_worker(i);
            }
        }
    }
