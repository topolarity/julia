...
/// Non-panicking variant: `None` when called from a non-worker thread
/// (e.g. a mutator doing lazy sweeping).
pub(crate) fn try_current_worker_ordinal() -> Option<ThreadId> {
    let ordinal = WORKER_ORDINAL.with(|x| x.load(Ordering::Relaxed));
    (ordinal != ThreadId::MAX).then_some(ordinal)
}
        ...
        loop {
            ...
            crate::policy::immix::immixspace::flush_live_bytes_tls();
            {
                ...
                if crate::diag::PAUSE_ACTIVE.load(O::Relaxed) {
                    ...
                    crate::diag::LAST_PKT_END_NS.store(crate::diag::now_ns(), O::Relaxed);
                }
                if crate::diag::PAUSE_ACTIVE.load(O::Relaxed)
                    && __d > crate::diag::PAUSE_PKT_REPORT_NS.load(O::Relaxed)
                {
                    eprintln!(
                        "[pause-pkt] t={}s {} {}us",
                        crate::diag::now_ns() / 1_000_000_000,
                        typename,
                        __d / 1000
                    );
                }
                if crate::diag::REQ_PENDING.load(O::Relaxed) {
                    crate::diag::PKTS_SINCE_REQ.fetch_add(1, O::Relaxed);
                    crate::diag::PKT_SUM_IN_WIN_NS.fetch_add(__d, O::Relaxed);
                    crate::diag::record_max(&crate::diag::PKT_MAX_IN_WIN_NS, __d);
                }
            }
        }
        debug!(
