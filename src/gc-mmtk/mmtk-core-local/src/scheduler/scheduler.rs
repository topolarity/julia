...
impl<VM: VMBinding> GCWorkScheduler<VM> {
    ...
    fn poll_slow(&self, worker: &GCWorker<VM>) -> PollResult<VM> {
        loop {
            ...
            self.worker_monitor.park_and_wait(
                ...
                // Spin-exit condition: this worker's own designated queue
                // (invisible to bucket polling) or any open bucket with
                // packets.  Direct state checks, so spinners exit exactly
                // when work is actually available.
                || {
                    !worker.shared.designated_work.is_empty()
                        || self.count_pending_packets() > 0
                },
            )?;
        }
    }
    ...
    fn on_last_parked(&self, worker: &GCWorker<VM>, goals: &mut WorkerGoals) -> LastParkedResult {
        ...
        match current_goal {
            WorkerGoal::Gc => {
                ...
                // (assertion relaxed: concurrent GC allows mid-cycle requests)
                ...
                // Transition-gap accounting: how long since the last packet
                // finished?  This is the park-convergence latency of the
                // all-park barrier.
                {
                    use std::sync::atomic::Ordering as O;
                    if crate::diag::PAUSE_ACTIVE.load(O::Relaxed) {
                        let last = crate::diag::LAST_PKT_END_NS.load(O::Relaxed);
                        if last != 0 {
                            let gap = crate::diag::now_ns().saturating_sub(last);
                            crate::diag::TRANS_GAP_SUM_NS.fetch_add(gap, O::Relaxed);
                            crate::diag::record_max(&crate::diag::TRANS_GAP_MAX_NS, gap);
                            crate::diag::TRANS_N.fetch_add(1, O::Relaxed);
                        }
                    }
                }
                ...
                // Churn detector: are we finding "more work" repeatedly with
                // no packet actually executed in between?
                let mut designated_churn = false;
                {
                    use std::sync::atomic::Ordering as O;
                    if crate::diag::PAUSE_ACTIVE.load(O::Relaxed) {
                        let cur = crate::diag::PAUSE_PKT_N.load(O::Relaxed);
                        let prev = crate::diag::LAST_SEEN_PKT_N.swap(cur, O::Relaxed);
                        if cur == prev && found_more_work.is_some() {
                            crate::diag::CHURN_N.fetch_add(1, O::Relaxed);
                            designated_churn =
                                crate::diag::LAST_FIND_BRANCH.load(O::Relaxed) == 1;
                        }
                    }
                }

                // ANTI-CHURN: pending *designated* work with no progress since
                // the previous all-park means a wake wave is already in
                // flight; the owners are runnable and will drain their queues
                // as soon as they are scheduled.  Re-broadcasting here only
                // lets already-awake workers barge the monitor mutex and
                // oscillate park/unpark, starving the owners (measured:
                // hundreds of no-progress all-park events per pause).  Park
                // and let the wave land.
                if designated_churn {
                    return LastParkedResult::ParkSelf;
                }

                if let Some(n) = found_more_work {
                    // PROPORTIONAL WAKEUP: a bucket transition with one packet
                    // (e.g. the global Prepare/Release packets) needs no futex
                    // traffic at all -- the last parked worker takes it.
                    // Fan-out inside packets wakes more workers via
                    // `WorkBucket::add`'s notify_one.
                    if n <= 1 {
                    ...
                    if concurrent_work_scheduled {
                        ...
                        // LEG 1: the plan's `end_of_gc` may have queued
                        // deferred post-pause packets with no-notify (it runs
                        // under the monitor mutex, where notifying would
                        // self-deadlock).  Wake workers for them here; the
                        // pause is over by now, so the crew cap no longer
                        // applies.
                        let pending = self.count_pending_packets();
                        if pending > 0 {
                            LastParkedResult::WakeN(pending + 1)
                        } else {
    ...
    fn respond_to_requests(
        ...
        let Some(goal) = goals.poll_next_goal() else {
            // FIX 1: trigger the final-mark pause on *worker readiness* rather than waiting for a
            // mutator to notice that the Concurrent bucket's queue looked empty at some poll site.
            //
            // Reaching here means: no goal is active, `find_more_work_for_workers` found nothing,
            // and every worker has parked -- i.e. concurrent marking is genuinely finished, with
            // zero packets in flight. That is the true condition the `is_drained()` proxy was
            // approximating, so raise the request ourselves instead of parking and waiting.
            use std::sync::atomic::Ordering as DO;
            crate::diag::NOREQ_PARKS.fetch_add(1, DO::SeqCst);
            let cp_opt = worker.mmtk.get_plan().concurrent();
            if cp_opt.is_some() {
            ...
            if ready_for_final_mark {
                crate::diag::SELF_TRIGGERED.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
                crate::diag::PAUSE_PENDING.store(false, std::sync::atomic::Ordering::SeqCst);
                goals.set_request(WorkerGoal::Gc);
                let Some(WorkerGoal::Gc) = goals.poll_next_goal() else {
                    return LastParkedResult::ParkSelf;
                };
                probe!(mmtk, gc_start);
                {
                    let mut gc_start_time = worker.mmtk.state.gc_start_time.borrow_mut();
                    if gc_start_time.is_none() {
        ...
        match goal {
            WorkerGoal::Gc => {
                ...
                {
                    use std::sync::atomic::Ordering as O;
                    let r = crate::diag::REQUEST_NS.load(O::SeqCst);
                    if r != 0 {
                        let d = crate::diag::now_ns().saturating_sub(r);
                        crate::diag::LAT_TOTAL_NS.fetch_add(d, O::SeqCst);
                        crate::diag::record_max(&crate::diag::LAT_MAX_NS, d);
                        crate::diag::LAT_COUNT.fetch_add(1, O::SeqCst);
                        crate::diag::REQ_PENDING.store(false, O::SeqCst);
                        crate::diag::PAUSE_PENDING.store(false, O::SeqCst);
                        let pk = crate::diag::PKTS_SINCE_REQ.swap(0, O::SeqCst) as u64;
                        crate::diag::PKTS_TOTAL.fetch_add(pk, O::SeqCst);
                    }
                }
                self.add_schedule_collection_packet();
                LastParkedResult::WakeSelf
    ...
    fn find_more_work_for_workers(&self) -> Option<usize> {
        use std::sync::atomic::Ordering as O;
        if self.worker_group.has_designated_work() {
            trace!("Some workers have designated work.");
            crate::diag::LAST_FIND_BRANCH.store(1, O::Relaxed);
            // Designated work is per-worker; wake everyone.
        ...
        if self.schedule_sentinels() {
            ...
            crate::diag::LAST_FIND_BRANCH.store(2, O::Relaxed);
            return Some(self.count_pending_packets().max(1));
        }

        // Try to open new buckets.
        if self.update_buckets() {
            trace!("Some buckets are opened.");
            crate::diag::LAST_FIND_BRANCH.store(3, O::Relaxed);
            return Some(self.count_pending_packets().max(1));
        }
    ...
    /// Approximate number of packets available in open buckets.  Used to size
    /// proportional wakeups at bucket transitions.
    fn count_pending_packets(&self) -> usize {
        self.work_buckets
            .iter()
            .map(|(_, b)| {
                if b.is_enabled() && b.is_open() {
                    b.num_packets()
                } else {
                    0
                }
    ...
    fn on_gc_finished(&self, worker: &GCWorker<VM>) -> bool {
        crate::diag::GC_FINISHED_ENTRY_NS
            .store(crate::diag::now_ns(), std::sync::atomic::Ordering::Relaxed);
