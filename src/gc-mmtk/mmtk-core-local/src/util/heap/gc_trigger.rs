...
impl<VM: VMBinding> GCTrigger<VM> {
    ...
    /// ASYNC TRIGGER: also `pub(crate)` so a concurrent plan's `collection_required`
    /// can request a cycle WITHOUT telling the poll chain to block the allocating
    /// mutator (request-and-continue; the mutator is stopped later by the pause's
    /// safepoint like everyone else).
