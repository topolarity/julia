...
impl<VM: VMBinding, B: Region> BlockPageResource<VM, B> {
    ...
    /// FIX E: batched form of `release_block` for mutator-side lazy sweeping.
    pub fn release_blocks_batch(&self, blocks: &[B]) {
        let pages = (1usize << Self::LOG_PAGES) * blocks.len();
        debug_assert!(pages <= self.common().accounting.get_committed_pages());
        self.common().accounting.release(pages as _);
        self.block_queue.push_batch(blocks)
    }
...
impl<B: Region> BlockPool<B> {
    ...
    pub fn push(&self, block: B) {
        ...
        // FIX E: lazy sweeping releases blocks from *mutator* threads, which
        // have no worker ordinal.  Route their blocks to the global queue.
        let Some(id) = crate::scheduler::try_current_worker_ordinal() else {
    ...
    /// FIX E: push many blocks from a non-worker (mutator) thread.  Fills
    /// queues to capacity and takes the global lock once, instead of one
    /// singleton-queue allocation + write-lock per block.
    pub fn push_batch(&self, blocks: &[B]) {
        self.count.fetch_add(blocks.len(), Ordering::SeqCst);
        let mut full_queues = Vec::new();
        let mut queue = BlockQueue::new();
        for &block in blocks {
            if let Err(block) = unsafe { queue.push_relaxed(block) } {
                full_queues.push(std::mem::replace(&mut queue, BlockQueue::new()));
                let result = unsafe { queue.push_relaxed(block) };
                debug_assert!(result.is_ok());
            }
        }
        let mut global = self.global_freed_blocks.write();
        global.append(&mut full_queues);
        if !queue.is_empty() {
            global.push(queue);
        }
    }
