...
pub trait Plan: 'static + HasSpaces + Sync + Downcast {
    ...
    /// FEWER WAKE EDGES: whether this GC needs the per-worker
    /// `PrepareCollector`/`ReleaseCollector` packets (copy-context reset).
    /// Waking every worker twice per pause just to reset unused copy
    /// allocators dominates short pauses; a plan may return false for pauses
    /// in which no copying can occur.
    ...
    fn release_nonmoving_space(&mut self, _full_heap: bool) {
        cfg_if::cfg_if! {
            if #[cfg(feature = "immortal_as_nonmoving")] {
                ...
                self.nonmoving.release(_full_heap, UnlogBitsOperation::NoOp, false);
            }
        }
    }
